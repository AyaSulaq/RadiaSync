...
class Base():
    ...
    #     """
    #     Rotate images and return [x, x_90, x_180, x_270] depending on the augmentation number choice.
    #     Augmentation Number Choices: 'Four', 'Two', 'One'
    #   - 'Four': Rotate by 90, 180, and 270 degrees.
    #   - 'Two': Randomly pick two rotations from 90, 180, and 270 degrees.
    #   - 'One': Randomly pick one rotation from 90, 180, and 270 degrees.
    #    """
    # # Convert input tensor x to float32 for compatibility with Kornia and CUDA operations
    #     x = x.to(dtype=torch.float32, device=self.device)

    #     out = [x]  # Start with the original image
    #     if num_augmentation == 'four':
    #         angles = [90, 180, 270]  # Fixed angles for 'four' augmentation
    #     elif num_augmentation == 'two':
    #         angles = random.sample([90, 180, 270], k=2)  # Randomly pick two unique angles
    #     elif num_augmentation == 'one':
    #         angles = [random.choice([90, 180, 270])]  # Randomly pick one angle
    #     else:
    #         raise NotImplementedError('Augmentation number not implemented yet')

    # # Generate rotated images for each angle
    #     for angle in angles:
    #         angle_tensor = torch.full((self.batch_size,), angle, dtype=torch.float32, device=self.device)
    #         rot_x = kt.rotate(x, angle_tensor)
    #         if contraD and num_augmentation in ['one', 'two']:
    #           out.append(rot_x)
    #         else:
    #           out.append(rot_x)

    #         if contraD and num_augmentation in ['one', 'two']:
    #           return tuple(out[1:])  # Return only the rotated images as tuple if contraD is True
    #         else:
    #           return torch.cat(out, 0)  # Concatenate along the batch dimension and return
