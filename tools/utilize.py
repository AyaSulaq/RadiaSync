...
def save_model(model, file_path,para_dict,  psnr): #para_dict , ssim, fid, kaid):
  ...
  ##ONLY PSNR
    model_path = '{}/best_model_{}_{}_{:.4f}.pth'.format(
        file_path, para_dict['source_domain'], para_dict['target_domain'], psnr) # fid, kaid)
...
def load_model(model, file_path, description):
    ...
    model_path = glob.glob('{}/checkpoint/{}/*.pth'.format(file_path, description))
...
def average(l):
       return sum(l) / (len(l)+1 ) ### H: if-statement plus one was added to prevent division by zero
